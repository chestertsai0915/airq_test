...
class DatabaseHandler:
    ...
    def save_market_data(self, symbol, interval, df):
        ...
        try:
            ...
            #這行指令會把整欄轉成乾淨的整數
            if df_to_save['open_time'].dt.tz is not None:
                df_to_save['open_time'] = df_to_save['open_time'].dt.tz_localize(None)
            df_to_save['open_time'] = (df_to_save['open_time'] - pd.Timestamp("1970-01-01")) // pd.Timedelta('1ms')
            ...
            if 'close_time' in df_to_save.columns:
                 ...
                 if df_to_save['close_time'].dt.tz is not None:
                     df_to_save['close_time'] = df_to_save['close_time'].dt.tz_localize(None)
                 df_to_save['close_time'] = (df_to_save['close_time'] - pd.Timestamp("1970-01-01")) // pd.Timedelta('1ms')
            else:
